...
import {  MdFoodBank,MdFavoriteBorder, MdFavorite } from "react-icons/md";
...
const ChefRecipes = () => {
  ...
  const { name, photo_url, years_of_experience, recipe_count } = chefDetails;
  return (
    <div>
      <div
        ...
        <div className="grid md:grid-cols-3 gap-3 mt-5">
          {chefRecipes.map(
            ({ id, cooking_method, ingredients, name, rating }) => (
              <div className="p-5 rounded-md shadow-md border self-start" key={id}>
                <div>
                <h3 className="font-bold text-xl">{name}</h3>
                </div>
                <div className="flex flex-wrap gap-2 my-3">
                  {ingredients.map((ingredient) => (
                    <span className="px-2 py-1 border text-xs bg-gray-200 rounded-full" key={ingredient}>
                      {ingredient}
                    </span>
                  ))}
                </div>
                <div>
                  ...
                  <ul className="list-disc ml-5">
                    {cooking_method.map((step) => (
                      <li className="text-sm" key={step}>{step}</li>
                    ))}
                  </ul>
                </div>
                ...
                <button className="btn btn-secondary btn-outline gap-2 mt-5"><MdFavorite size={20}/> Make It Favourite</button>
              </div>
            )
          )}
        </div>
